...
constexpr std::size_t kTestPayloadBytes = 60;
constexpr std::size_t kTestPacketBytes = pr1::kHeaderSize + kTestPayloadBytes;
constexpr uint8_t kFlagTestPattern = 0x80;

static_assert(kTestPacketBytes <= 127,
              "M0 deterministic packet must fit a FLRC payload budget");
...
constexpr uint8_t kFlrcCodingRate = 3;  // RadioLib: coding rate 3/4.
...
bool InitRadio() {
  ...
  // This overload is intentionally explicit so no frequency/power default is
  // hidden from the test log. RadioLib currently supports these parameters on
  // SX1280. The final legal/certified configuration is NOT decided here.
  ...
  // Vendor MVSR pin_config.h: LORA_RX=21, LORA_TX=10.
  // RadioLib expects the RX-enable pin first, TX-enable pin second.
...
#if PR1_NODE_ROLE_TX
  TransmitOne();
  delay(100);  // 10 packets/s M0 baseline. Audio-rate traffic comes later.
#else
  ReceiveOne();
#endif
#else
