...
window. It therefore cannot infer packets lost before the first observed packet
or after the final observed packet. For a controlled test, compare this summary
with the TX-side sent count as well.
"""
...
def analyze_lines(lines: Iterable[str]) -> Summary:
    ...
    for line in lines:
        ...
        if delta == 0:
            ...
            # Forward progress, including normal uint16 wrap-around.
            if delta > 1:
                summary.sequence_missing_inside_window += delta - 1
            latest_forward_seq = packet_seq
        else:
            # More than half the uint16 sequence space backwards is treated as
            # an old/reordered packet, not as 65k missing packets.
...
def print_human(summary: Summary) -> None:
    ...
    if summary.rssi_avg_dbm is not None:
        print(f"  RSSI avg/min:                    {summary.rssi_avg_dbm:.2f} / {summary.rssi_min_dbm:.2f} dBm")
    if summary.snr_avg_db is not None:
        print(f"  SNR avg/min:                     {summary.snr_avg_db:.2f} / {summary.snr_min_db:.2f} dB")
